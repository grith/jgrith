package org.vpac.security.light.voms.VOManagement;
...
public class GroupManagement {
	...
	static final Logger myLogger = Logger.getLogger(GroupManagement.class.getName());
	
	/**
	 * Returns all fqans of a user with the specified credential for the vo
	 * @param vo the vo you want to know the fqans for
	 * @param gssCred the credential
	 * @return all fqans of this user for the vo or null if the user is not a member of the VO
	 */
	public static String[] getAllFqansForVO(VO vo, GSSCredential gssCred) {
		...
		try {
			globusCredential = CredentialHelpers.unwrapGlobusCredential(gssCred);
			
			// create a temporary VomsProxyCredential to contact the voms server
			// and ask about all it knows about the user
			vomsProxy = new VomsProxyCredential(globusCredential, vo, "A", null, 1);
			...
			if ( status == VOManagement.MEMBER ) {
				ArrayList<String> fqans = new VOMSAttributeCertificate(vomsProxy.getAttributeCertificate()).getVOMSFQANs();
				vomsProxy.destroy();
	...
	public static String getRolePart(String fqan) {
		String role = fqan.substring(fqan.indexOf("/Role=")+6, fqan.indexOf("/Capability="));
		return role;
	}
	
//	public static String getStatusPart(String fqan) {
//		return fqan.substring(fqan.indexOf(" STATUS:")+8, fqan.indexOf(" REASON:"));
//	}
