...
package org.vpac.security.light.voms;


/**
 * A VO consist of the name, a host the management software (VOMS/VOMRS) is hosted, the port to contact the VOMS
 * server (for voms proxy creation) and the DN of the host. Also the url for the endpoint of the VOMRS webservice.
 * At the moment this url is created throught String concatenation in a dodgy way. So, unless your VOMRS server is 
 * set up in a way to accomodate that, don't rely on it.
 * 
...
public class VO implements Comparable<VO> {
	...
	/**
	 * The default constructor. As said above, the vomrs_url is created in a dodgy way, so beware.
	...
	public VO (String voName, String host, int port, String hostDN){
		...
		this.port = port;
		this.hostDN = hostDN;
		this.vomrs_url = "https://"+host+":8443/vo/"+voName+"/services/VOMRS?wsdl";
	}
...
//		//TODO 
//		return new VO("Chris", "vomrsdev.vpac.org", 15003, "/C=AU/O=APACGrid/OU=VPAC/CN=vomrsdev.vpac.org");
//	}
	
	public VO (String voName, String host, int port, String hostDn, String vomrsURL) {
		this.voName = voName;
		this.host = host;
		this.port = port;
		this.hostDN = hostDn;
