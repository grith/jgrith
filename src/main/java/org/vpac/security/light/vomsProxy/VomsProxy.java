...
/**
 * A VomsProxy is a wrapper for a {@link GlobusCredential} that has got a voms attribute 
 * certificate attached to it. This class also provides all the methods to easily create such a proxy
 * in Java.
 * 
 * Internally this class uses a {@link GlobusCredential} as a kind of "base" credential which will be created
 * prior to getting the voms attribute certificate. This "base" proxy is used to contact the voms server which in 
 * turn will issue the attribute certificate for the group the user requested (if he is a member, that is).
 * 
 * The magic happens in the classes {@link VomsProxyCredential} and {@link VOMSAttributeCertificate}.
 * 
 * At the moment you only can specify the (sub-)group you want the proxy to have information included. If we should need
 * more detailed information in the proxy I'll change this so that you can specify the command to the voms server
 * directly.
 * 
...
public class VomsProxy {
	...
	public VomsProxy(File proxyFile) throws Exception{
		...
		try {
			...
			myLogger.error("Could not load valid credential from file \""+proxyFile.toString()+"\": "+e.getMessage());
		}
	...
	/**
	 * This creates a VomsProxy using usercert.pem and userkey.pem in the default
	 * globus location to create a intermediate proxy that is used to attach the 
	 * voms attribute certificate to. You have to provide the passphrase of the private 
	 * key.
	 * 
	 * @param vo the vo you want to have the proxy for
	 * @param group the group you want to have the proxy for (example: /APACGrid/NGAdmin)
	 * @param passphrase the passphrase of the private key
	 * @param lifetime_in_ms the lifetime of the proxy (and the voms attribute certificate) in milliseconds
	 * @throws Exception 
	 */
	public VomsProxy(VO vo, String group, char[] passphrase, long lifetime_in_ms) throws Exception{
		...
		//		G/group 	- 	This means get group informations.  /group should be /vo-name.
        //						This is the default request used by voms-proxy-init
		//		Rrole		-	This means grant me the specified role, in all groups in which you can grant it.
		...
		try {
			...
			myLogger.error("Could not create voms attribute certificate: "+e.getMessage());
			throw e;
		}
	...
	/**
	 * Creates a VomsProxy using the {@link GlobusCredential} you provide. The constructor tries to contact
	 * the voms server and get an attribute certificate for the requested VO/group and then attaches this 
	 * attribute certificate to the {@link GlobusCredential}.
	 * 
	 * Use getVomsProxyCredential() to get the actual voms proxy.
	 * 
	 * @param vo the VO you want the proxy forprivate
	 * @param group the (sub-)group within the VO you want the proxy for (example: /APACGrid/NGAdmin)
	 * @param baseProxy a valid GlobusCredential (with the appropriate dn the voms server has in it's database)
	 * @param lifetime_in_ms the lifetime of the proxy (and the voms attribute certificate) in milliseconds
	 * @throws Exception if somethings gone wrong
	 */
	public VomsProxy(VO vo, String group, GlobusCredential baseProxy, long lifetime_in_ms) throws Exception {
		...
		try {
			...
			myLogger.error("Can't create Voms attribute certificate because base proxy is not valid: "+e.getMessage());
			throw new Exception("Can't create Voms attribute certificate because base proxy is not valid: "+e.getMessage());
		}
	...
	private void createAttributeCertificate(GlobusCredential baseCred, VO vo, long lifetime_in_ms) throws Exception {

			// create the VomsProxyCredential
			vomsProxyCredential = new VomsProxyCredential(baseCred, vo, command, order, new Long(lifetime_in_ms/1000).intValue());
	...
	/**
	 * Create a "plain" GlobusCredential first which is used to attach the 
	 * VOMS Attribute certificate to
	 * @param passphrase the passphrase of the private key
	 ...
	 * @throws Exception 
	 */
	private void createDefaultBaseProxy(char[] passphrase, long lifetime_in_ms) throws Exception {
	 ...
	 * 
	 * @return the {@link GlobusCredential} with the attached voms attribute certificate or null if something went wrong during creation
	 */
	 ...
	 * @param fullString whether you want the full String or only VO and group
	...
	public String getDefaultFqan(boolean fullString) {
		...
		try {
			VOMSAttributeCertificate vomsac = new VOMSAttributeCertificate(vomsProxyCredential.getAttributeCertificate());
			...
			myLogger.error("Couldn't get default fqan of VomsProxy: "+e.getLocalizedMessage());
			return null;
		}
	...
	/**
	 * This one creates a voms proxy with the requested vo information and returns it
	 * as GSSCredential. It's a convenience method so you don't have to create a VomsProxy wrapper
	 * object if you don't need to.
	 * 
	 * @param vo
	...
	public static GSSCredential init(VO vo, String group, char[] passphrase,
		...
		try {
			...
			LocalVomsProxy.myLogger.error("Could not create voms proxy: " + e.getMessage());
			throw e;
		}
	
		if (vomsproxy == null || vomsproxy.getVomsProxyCredential() == null) {
			LocalVomsProxy.myLogger.error("Voms proxy is null.");
			throw new NullPointerException("VomsProxy or VomsProxyCredentail is null.");
		}
	
		try {
			vomsproxy.getVomsProxyCredential().verify();
		} catch (GlobusCredentialException e) {
			LocalVomsProxy.myLogger.error("Voms proxy is not valid: " + e.getMessage());
			throw e;
		}
	
		return CredentialHelpers.wrapGlobusCredential(vomsproxy.getVomsProxyCredential());
	}
